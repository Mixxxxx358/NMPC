...
def NMPC(system, encoder, x_min, x_max, u_min, u_max, x0, u_ref, Q, R, dt, dlam, stages, x_reference_list, Nc=5, Nsim=30, max_iterations=1):
    ...
    x = np.repeat(np.array([x0_norm]).T, Nc+1, axis=1)
    ...
    for mpciter in np.arange(Nsim):
        # set reference for current mpciter
        ...
        x0_norm = (x_denormalized - norm.y0)/norm.ystd

        x_log[:,mpciter+1] = x_denormalized
    ...
    return x_log, u_log, t, runtime, lpv_counter, reference_list_normalized
...
if __name__ == "__main__":
    ...
    Nsim_remaining = Nsim
    while True:
        Nsim_steps = random.randint(5,10)
    ...
    plt.plot(np.arange(Nsim+1)*dt, np.hstack((np.array(x0[1]),x_reference_list[:Nsim])), label='reference') # figure out what the correct hstack should be here
