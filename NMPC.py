...
def NMPC(system, encoder, x_min, x_max, u_min, u_max, x0, u_ref, Q, R, dt, dlam, stages, x_reference_list, Nc=5, Nsim=30, max_iterations=1):
    ...
    #[A_sym, B_sym] = par_lpv_int(x,n_states,u,n_controls,Jfx,Jfu,dlam,stages,L)
    ...
    components_times = np.zeros(4) # getAB, solve, misc, denorm and sim
    x_log[:,0] = x0
    start = time.time()
    ...
    for mpciter in np.arange(Nsim):
        ...
        while True:
            ...
            components_times[0] = components_times[0] + time.time() - component_start

            # solve for u and x
            component_start = time.time()
            sol = opti.solve()
            components_times[1] = components_times[1] + time.time() - component_start

            # take u and x out of solve
            component_start = time.time()
            ...
            components_times[2] = components_times[2] + time.time() - component_start
        ...
        components_times[3] = components_times[3] + time.time() - component_start
...
if __name__ == "__main__":
    ...
    x_log, u_log, e_log, comp_t_log, t, runtime, lpv_counter, reference_list_normalized, components_times = NMPC(sys_unblanced, I_enc, \
    ...
    fig1 = plt.figure(figsize=[14.0, 6.0])
    ...
    fig2 = plt.figure(figsize=[14.0, 6.0])
    plt.bar(['getAB', 'solve', 'overhead', 'simulate'], components_times)
